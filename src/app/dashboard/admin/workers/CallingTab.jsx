...
// The calling/assignment view of the merged Workers page — this is the
// former standalone Contacts page (src/app/dashboard/admin/contacts/page.js),
// relocated as-is into a Workers tab. Still reads/writes the `contacts`
// table through the exact same /api/contacts/** endpoints; only the page
// chrome (header/breadcrumb/tab switcher) now lives in the parent
// Workers page. Every contact is guaranteed to have a linked worker_id
// (backfilled via scripts/backfill-contact-workers.mjs), so a name click
// opens the same worker detail popup the Directory tab uses.
export default function CallingTab({ session }) {
...
// "All …" / "N selected", opens a checkbox panel via FloatingPopover so it
// stays fully on-screen instead of clipping on mobile or inside a scrolled
// filter bar.
function FilterMultiSelect({ label, items, selected, onChange, disabled, disabledLabel }) {
