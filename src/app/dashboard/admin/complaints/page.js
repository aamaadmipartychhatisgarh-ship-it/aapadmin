...
import { MessageSquare, Plus, Loader2, X, Droplet, Construction, Zap, Package, HelpCircle, Pencil } from "lucide-react";
...
function Body() {
  const [data, setData] = useState({ complaints: [], counts: {}, byType: [] });
...
function AddModal({ onClose, onSaved, editing }) {
    ...
    <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-md p-6 space-y-3">
        ...
        <div className="flex justify-end gap-2 pt-2">
          ...
          <button onClick={save} disabled={saving || !form.citizen_name} className="px-4 py-2 text-sm bg-[#164FA3] hover:bg-blue-800 disabled:opacity-50 text-white rounded-lg font-semibold">{saving ? "Saving…" : (editing ? "Save" : "Log")}</button>
        </div>
      </div>
    </div>
  );
