...
// Supervisor's own Contacts Distribution & Call Assignment module — same UI,
// workflow and features as the Super Admin Contacts page
// (src/app/dashboard/admin/contacts/page.js), but every request is scoped
// server-side to the signed-in supervisor's own territory (see
// src/lib/supervisorScope.js + the /api/supervisor/contacts* routes). The
// frontend never enforces the boundary on its own — every fetch below hits a
// dedicated supervisor-only endpoint that re-checks scope on the server, so
...
function Body({ session }) {
  ...
  const [territory, setTerritory] = useState(null); // { level, zone, lok_sabha, district, assembly }

  const [contacts, setContacts] = useState([]);
  const [total, setTotal] = useState(0);
  const [users, setUsers] = useState([]); // callers in territory
  ...
  // Geo filters — pre-seeded/locked to the supervisor's own territory once
  // it resolves (see the `territory` effect below).
  ...
  const [bulkMode, setBulkMode] = useState("even");
  ...
  // Resolve the supervisor's own territory once, then seed the geo filters
  // to it (locking the levels at/above the anchor — see the JSX below).
  useEffect(() => {
    fetch("/api/supervisor/contacts/scope").then((r) => r.json()).then((d) => {
  ...
  useEffect(() => {
    ...
    fetch("/api/supervisor/contacts/teams").then((r) => r.json()).then((d) => setTeams(d.teams || [])).catch(() => {});
  }, []);

  useEffect(() => { if (!scopeLoading) loadCounts(); }, [scopeLoading, zoneId, lokSabhaId, districtId, assemblyIds, designationIds]);
  async function loadCounts() {
    ...
    const fetchCount = async (status) => {
      ...
      p.set("status", status);
      const r = await fetch(`/api/supervisor/contacts?${p}`, { cache: "no-store" });
  ...
  async function loadTeamCallers(teamId) {
    setBulkTeam(teamId);
    if (!teamId) return;
    const r = await fetch(`/api/supervisor/contacts/teams/${teamId}`);
    if (!r.ok) return;
    const d = await r.json();
    const callerIds = users.map((u) => u.id);
    const memberCallerIds = (d.members || [])
      .filter((m) => callerIds.includes(m.user_id))
      .map((m) => m.user_id);
    setBulkCallers(memberCallerIds);
    if (memberCallerIds.length === 0) setError("This team has no caller accounts in your territory.");
    else setError("");
  }
  ...
  async function bulkRecall() {
    setMessage(""); setError("");
    const target = bulkCallers.length > 0 ? `${bulkCallers.length} selected caller(s)` : "ALL callers in your territory";
    if (!confirm(`Remove assigned contacts from ${target}? Contacts stay in the database and return to the pool.`)) return;
    setBulkBusy(true);
    try {
      const r = await fetch("/api/supervisor/contacts/bulk-unassign", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  // Cascading location filters BELOW the supervisor's locked anchor level —
  // identical cascade primitives to the admin page, just starting from the
  // supervisor's own territory instead of "All Zones".
  useEffect(() => {
    if (!territory || territory.level !== "zone") return; // only a zone-anchored supervisor cascades from here
  ...
  }, [territory, lokSabhaId]);
  useEffect(() => {
    if (!territory || territory.level === "assembly") return; // already at the finest grain
    let url = null;
  ...
  }, [territory, districtId, lokSabhaId]);
  ...
  async function fetchContactsPage(pageNum) {
    const r = await fetch(`/api/supervisor/contacts?${buildParams(pageNum)}`, { cache: "no-store" });
    if (!r.ok) return null;
    const d = await r.json();
  ...
  async function editStep(delta) {
    ...
    if (targetPage < 1 || (delta > 0 && (targetPage - 1) * PAGE_SIZE >= total)) return;
    setEditNavBusy(true);
  ...
  async function assign(contactId, userId) {
    const r = await fetch(`/api/supervisor/contacts/${contactId}`, {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
  ...
  if (!hasScope) {
    return (
      <div className="space-y-6 animate-in fade-in duration-500">
        <PageHeader icon={UserCheck} title="Contacts" breadcrumb={[{ label: "Dashboard", href: "/dashboard/supervisor" }, { label: "Contacts" }]} />
        <div className="bg-amber-50 border border-amber-200 text-amber-800 rounded-2xl p-6 text-center">
          ...
          <p className="font-semibold">No territory assigned yet.</p>
          <p className="text-sm mt-1">Ask your admin to set your Zone, District, or Assembly scope before you can manage contacts.</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500">
      <PageHeader
        ...
        breadcrumb={[{ label: "Dashboard", href: "/dashboard/supervisor" }, { label: "Contacts" }]}
      />
      ...
      <div className="flex items-center gap-3 flex-wrap">
        ...
        <select value={zoneId} disabled={!!territory?.zone} onChange={(e) => setZoneId(e.target.value)} className="h-9 px-3 rounded-lg border border-gray-200 text-sm bg-white disabled:bg-gray-50 disabled:text-gray-500">
          {territory?.zone ? <option value={territory.zone.id}>{territory.zone.name}</option> : <option value="">All zones</option>}
        </select>
        <select value={lokSabhaId} disabled={territory?.level !== "zone"} onChange={(e) => setLokSabhaId(e.target.value)} className="h-9 px-3 rounded-lg border border-gray-200 text-sm bg-white disabled:bg-gray-50 disabled:text-gray-500">
          {territory?.level === "zone" ? (
            <>
        ...
        <select value={districtId} disabled={territory?.level !== "zone"} onChange={(e) => setDistrictId(e.target.value)} className="h-9 px-3 rounded-lg border border-gray-200 text-sm bg-white disabled:bg-gray-50 disabled:text-gray-500">
          {territory?.level === "zone" ? (
            <>
        ...
        {territory?.level === "assembly" ? (
          ...
          <FilterMultiSelect
            ...
            disabled={!districtId && !lokSabhaId} disabledLabel="Pick district"
          />
        )}
      ...
      <CollapsibleSection
        ...
        storageKey="supervisor_contacts_bulk_distribute"
      ...
      >
        ...
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm text-gray-800 font-semibold">
            Distribute the {total.toLocaleString()} {filter !== "all" ? filter + " " : ""}contacts in your territory
        ...
        <div className="flex flex-wrap gap-2">
          {users.length === 0 && <span className="text-xs text-gray-500">No callers in your territory yet.</span>}
          {users.map((u) => {
        ...
        <div className="flex items-center gap-3 flex-wrap">
          ...
          <span className="text-xs text-gray-500">
            {filter === "pending"
              ...
              : "Distributes every contact matching the current status filter, including Done ones."}
          </span>
        </div>
        ...
        <div className="flex items-center gap-3 flex-wrap pt-2 border-t border-blue-100">
          <button onClick={bulkRecall} disabled={bulkBusy} className="inline-flex items-center gap-2 bg-white border border-red-200 text-red-600 hover:bg-red-50 disabled:opacity-50 px-4 py-2 rounded-xl text-sm font-semibold">
            <UserMinus size={16} />
            {bulkCallers.length > 0 ? `Remove contacts from ${bulkCallers.length} selected caller(s)` : "Remove contacts from ALL callers in your territory"}
          </button>
...
// Simplified vs. the admin Edit modal — geography isn't editable here (see
// the comment in src/app/api/supervisor/contacts/[id]/route.js for why: a
// supervisor may not relocate a contact's territory, only who owns it).
function EditContactModal({ contact, position, total, hasPrev, hasNext, navBusy, onPrev, onNext, onClose, onSaved }) {
  ...
  async function save() {
    setSaving(true); setError("");
    const body = {
      person_name: form.person_name, phone_number: form.phone_number,
      address: form.address, designation_id: form.designation_id || null,
    };
    const r = await fetch(`/api/supervisor/contacts/${contact.id}`, {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
