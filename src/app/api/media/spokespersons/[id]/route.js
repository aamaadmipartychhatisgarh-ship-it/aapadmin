...
export async function PUT(req, { params }) {
  try {
    ...
    // photo_url editable (BUG 26): sending "" clears ONLY the photo (§8), other
    // fields are updated only when present so existing data is never lost (§5).
    const fields = ["name", "mobile", "expertise", "languages", "photo_url", "is_active"];
    const sets = [], vals = [];
