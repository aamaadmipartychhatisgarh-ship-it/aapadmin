...
export async function PUT(req, { params }) {
  ...
  try {
    ...
    const res = await query(
      `UPDATE la_mla_elections SET election_year=?, election_type=?, party=?, candidate=?, status=?, votes=?, vote_percentage=?, margin=?, result=?, runner_up=?, runner_up_votes=?, person_category=? WHERE id=?`,
      [numOrNull(d.election_year), strOrNull(d.election_type), strOrNull(d.party), strOrNull(d.candidate),
       ...
       strOrNull(d.runner_up), numOrNull(d.runner_up_votes), strOrNull(d.person_category), eid]
    );
